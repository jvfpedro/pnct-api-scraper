...
caminho = r"D:\UFSC\PI\python\volume_hora\postos"
...
for posto, ano in postos.items():
    ...
    for mes, dia_max in dicionario_dias_mes.items():
        ...
        while dia <= dia_max:
            url = f"https://servicos.dnit.gov.br/dadospnct/api/VolumeHora/{posto}?ano={ano}&mes={mes}&dia={dia}&_={time()}"
            ...
            if "dado" in response.json() and response.json()["dado"]:
                ...
                df = df[["idEquipamento", "sentido", "ano", "mes", "dia", "hora", "valorVH"]]
